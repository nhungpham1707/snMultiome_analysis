...
# load data
adata = sc.read('/hpc/pmc_drost/PROJECTS/cell_origin_NP/clean_code_bu/output/sc_RNA/merge_all/ecMRT.h5ad')
print('adata is')
adata
...
# save model 
# with open(save_dir+'ecMRT_model.pkl', 'wb') as file:
#     pickle.dump(model, file)

with open(save_dir+'ecMRT_model.pkl', 'rb') as file:
...
# fig.tight_layout()
fig.savefig(save_dir+'ecMRT_losses.png')
...
print('embed.obs[individual.id] is')
print(embed.obs['Individual.ID'])
# # Compute UMAP
# print('----compute umap------')
# sc.pp.neighbors(embed, use_rep="X")
# sc.tl.umap(embed)
# with open(save_dir+'ecMRT_embed.pkl', 'wb') as file:
#     pickle.dump(embed, file)


# with open(save_dir+'ecMRT_embed.pkl', 'rb') as file:
#     embed = pickle.load(file)
# # Plot UMAP embedding

# # Obs columns to color by
# cols = ["Individual.ID", "singler.pruned.labels"]
...
#         frameon=False,
#     )
# fig.savefig(save_dir+'ecMRT_umap.png')
